...
public class HealthWardBlock extends Block implements BlockEntityProvider
{
    ...
    {
        return new HealthWardBlockEntity(pos, state);
    }

    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type)
    {
        return HealthWardBlockEntity::tick;
    }

    @Environment(EnvType.CLIENT)
    public void appendTooltip(ItemStack stack, BlockView world, List<Text> tooltip, TooltipContext options)
    {
        tooltip.add(new TranslatableText("block.ward_blocks.health_ward_block.tip1").formatted(Formatting.GREEN));
    }
}